...
func (d *Daemon) doSync(logger log.Logger, lastKnownSyncMarkerRev *git.GitRef, warnedAboutSyncMarkerChange *bool) (retErr error) {
	...
	oldSyncMarkerRev, err := working.SyncMarkerRevision(ctx)
	...
	headRevision, err := working.HeadRevision(ctx)
	if err != nil {
		return err
	}
	...
	var commits []git.Commit
	{
		...
		ctx, cancel := context.WithTimeout(ctx, d.GitOpTimeout)
		if oldSyncMarkerRev != "" {
			commits, err = d.Repo.CommitsBetween(ctx, oldSyncMarkerRev, headRevision, d.GitConfig.Paths...)
		} else {
			initialSync = true
			commits, err = d.Repo.CommitsBefore(ctx, headRevision, d.GitConfig.Paths...)
		}
		cancel()
		if err != nil {
			return err
	...
	if oldSyncMarkerRev != headRevision {
		{
			ctx, cancel := context.WithTimeout(ctx, d.GitOpTimeout)
			syncMarkerAction := git.SyncMarkerAction{
				Revision: headRevision,
				Message:  "Sync pointer",
			}
			err := working.UpdateSyncMarker(ctx, syncMarkerAction)
			cancel()
			if err != nil {
				return err
			}
			*lastKnownSyncMarkerRev = headRevision
		}
		logger.Log("tag", d.GitConfig.SyncMarkerName, "old", oldSyncMarkerRev, "new", headRevision)
		{
			ctx, cancel := context.WithTimeout(ctx, d.GitOpTimeout)
			err := d.Repo.Refresh(ctx)
			cancel()
